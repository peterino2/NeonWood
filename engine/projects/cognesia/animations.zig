...
pub const SpriteAnimationInstance = struct {
    ...
    pub fn advance(self: *@This(), deltaTime: f64) void {
        ...
        if (self.currentFrameTime > animation.frameTime) {
            ...
            self.currentAnimFrameIndex = (self.currentAnimFrameIndex + 1) % animation.frameCount;
        ...
        core.assert(self.currentAnimFrameIndex != animation.frameCount);
    }
};
...
const SoundEventWrap = struct {
    ...
    pub fn exec(ptr: *anyopaque) void {
        ...
        audio.gSoundEngine.playSound(this.soundName) catch {
            audio.sound_err("unable to play sound {s}", .{this.soundName.utf8});
        };
    }
};
